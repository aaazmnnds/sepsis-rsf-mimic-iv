...
def run_deepsurv(X, y, learning_rate=1e-3):
    ...
    net = tt.practical.MLPVanilla(X.shape[1], [32, 32], 1)
    model = CoxPH(net, tt.optim.Adam(lr=learning_rate))
    model.fit(X, (y_time, y_event), batch_size=256, epochs=100, verbose=False)
    return model
...
def main():
    ...
    for mechanism, mice_files in mice_groups.items():
        ...
        m = len(mice_files)
        models_to_run = [("RSF", run_rsf), ("XGBoost", run_xgboost_survival),
    ...
    for file_path in single_files:
        ...
        for model_name, model_func in models_to_run:
            ...
            try:
                model = model_func(X_train_scaled, y_train)
                if model_name == "XGBoost": risk_scores = model.predict(xgb.DMatrix(X_test_scaled))
